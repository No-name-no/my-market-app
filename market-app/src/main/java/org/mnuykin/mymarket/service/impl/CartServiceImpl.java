...
public class CartServiceImpl implements CartService {
        ...
        return SecurityContextHolder
                ...
                .flatMap(user -> itemRepository.getItemById(id)
                        ...
                        .flatMap(item -> cartRepository.getCartItemByItemIdAndUserId(item.getId(), user.getId())
                                .defaultIfEmpty(new CartItem(null, user.getId(), item.getId(), 0))
                                .flatMap(cartItem -> {
                                    switch (action){
    ...
    private Flux<ItemDto> getItemsByUser(User user){
        return cartRepository.findAllByUserId(user.getId()).flatMap(cartItem -> {
